...
export function extractKnowledgeLocal(text, source = '手动录入', options = {}) {
  ...
  return sections.map(({ text: section, path, continuation }, index) => {
    ...
    return {
      ...
      title,
      content,
      category: options.category || inferCategory(`${title} ${content}`),
      tags: [...new Set([...(options.tags || []), ...inferTags(`${title} ${content}`)])].slice(0, 6),
      group: path.length > 1 ? path[1] : (path[0] || '未分类'),
      section: sectionPath || '未分类',
      status: 'pending',
      source,
